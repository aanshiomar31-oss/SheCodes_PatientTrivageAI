...
FastAPI app uses and `--autogenerate` sees every model in `app.models`.
"""
...
import app.models  # noqa: E402,F401  (registers all model tables on Base.metadata)
...
# Inject the live application DATABASE_URL instead of relying on a
# hard-coded value in alembic.ini.
